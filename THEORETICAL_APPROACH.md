...
## 3.8 Implications of the Model

We derive three qualitative implications that follow as logical consequences
of the equations presented above, for any admissible parameterization
satisfying $\chi > 0$, $\Delta R^L_{aut} > 0$, and $\lambda > 1$.

**Implication 1 (output cost ranking and intertemporal divergence).** Let
$\hat{y}^{nd}_h$ and $\hat{y}^{def}_h$ denote the output responses under
the two regimes at horizon $h$. The without-default regime is governed
entirely by the balance-sheet channel (LL.5a) and the working-capital channel
(LL.2a). The default regime adds two further terms: the autarky lending-rate
wedge $\Delta R^L_{aut} > 0$, which enters negatively in (LL.2a) at every
horizon, and the capital-depletion path $\hat{k}^{excl}_h < 0$ from (4'),
which also enters negatively through the term $\alpha\hat{k}^{excl}_h$ in
(LL.2a). Both additional terms are non-positive for all $h \geq 0$, so the
model implies $\hat{y}^{def}_h \leq \hat{y}^{nd}_h$ for all $h$, with strict
inequality for all $h \geq 1$ whenever $\chi > 0$. Furthermore, the
capital-depletion term in (4') compounds geometrically at rate
$(1-\chi\delta)^h$: it grows in absolute value as $h$ increases. The gap
$\hat{y}^{nd}_h - \hat{y}^{def}_h$ is therefore non-decreasing in $h$ — the
two output paths diverge over time rather than converge. This divergence is
a consequence of the autonomous capital-depletion dynamics in the default
regime, and cannot be generated by the working-capital or balance-sheet
channels alone, which are common to both regimes.

**Implication 2 (asymmetric channel dominance and the credit-to-GDP ratio).**
Equations (LL.2a), (LL.3a), (LL.5a), and (IR.4) imply that the dominant
source of persistent output deterioration differs structurally across the two
regimes. In the without-default regime, the capital stock $\hat{k}_h$
accumulates losses only through the lending-rate wedge in (LL.3a), which
depends on the spread path $\hat{s}^{nd}_h$. As the spread mean-reverts
after the crisis onset, the capital channel is bounded, and the dominant
driver of persistent output loss is the net-worth recursion (LL.5a). Private
credit, which loads entirely on net worth through (IR.4), therefore moves in
proportion to output: both are driven by the same balance-sheet shock. In the
default regime, $\hat{k}^{excl}_h$ follows the autonomous path (4'), which
is decoupled from the spread after the onset of autarky: capital continues to
erode at rate $\chi\delta$ regardless of whether the spread has normalized.
Output therefore has an additional negative component — capital depletion —
that credit does not share, since (IR.4) links credit to net worth, not to
the capital stock. It follows directly that the ratio of the credit
contraction to the output contraction is larger in the without-default regime
than in the default regime: the credit-to-GDP ratio falls more when spread
crises are resolved without default.

**Implication 3 (structural sources of output persistence).** In both
regimes, the model implies that output losses persist beyond the period during
which the sovereign spread is elevated, but for structurally distinct reasons.
In the without-default regime, equation (LL.3a) establishes that any
reduction in investment at horizon $h$ lowers the capital stock at $h+1$,
which reduces output at $h+1$ through the term $\alpha\hat{k}_h$ in (LL.2a),
even if the spread has already returned to its steady-state value $s_{ss}$.
The persistence of output losses is therefore bounded by the speed at which
the capital stock recovers, governed by the depreciation rate $\delta$ and
the investment-sensitivity parameter $\eta$. In the default regime, the
source of persistence is different: the capital-depletion path (4') is
independent of the spread after autarky begins, and output remains depressed
for as long as the sovereign is excluded from international markets. The
survival-weighted path (5) shows that the average output loss in this regime
decays at rate $(1-\mu)^h$, governed by the re-entry probability $\mu$ rather
than by any endogenous capital-adjustment process. The two regimes therefore
generate output persistence through separate and non-overlapping channels —
endogenous capital inertia in the without-default case, and exogenous
exclusion duration in the default case.
