/*===========================================================================
  08C_AIPW_MINDUR_ANALYTICSE.DO — DUPLICATE OF 08c_aipw_mindur.do,
  PAPER-ALIGNED SE VARIANT

  Standalone duplicate of 08c_aipw_mindur.do, NOT wired into 00_master.do.
  The ONE change: every level (nd/def) confidence interval below uses the
  paper's own ANALYTIC (unclustered influence-function) SE (`A1'/`A2',
  already returned by _aipwpair) instead of 08c_aipw_mindur.do's ADOPTED
  row-bootstrap SE (`BSE1'/`BSE2'). See 08b_aipw_analyticSE.do's header for
  the full rationale -- identical reasoning, applied here under the
  minimum-duration treatment definition. Point estimates are UNCHANGED
  (analytic vs. bootstrap only changes the SE/CI, never the point estimate
  itself). Outputs write to DIFFERENT filenames so this never overwrites
  08c_aipw_mindur.do's own results.
===========================================================================*/
...
* ══════════════════════════════════════════════════════════════════════════
* PROGRAMS — identical to 08c_aipw_mindur.do / 08b_aipw.do / 13c_aipw_channels.do
* ══════════════════════════════════════════════════════════════════════════
...
di as result _n "08c_aipw_mindur_analyticSE.do complete."
di as result "Compare against 08c_aipw_mindur.do's own outputs directly: same point"
di as result "estimates, level SE/CI swapped from row-bootstrap to the paper's own"
di as result "analytic formula."
