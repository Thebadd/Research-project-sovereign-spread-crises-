...
* ── Color scheme ────────────────────────────────────────────────────────────
* Main: navy blue (all/ND), cranberry (default-linked)
local c_all  "23 55 94"      // dark navy
local c_nd   "0 84 166"      // blue
local c_def  "157 36 73"     // dark red
local c_zero "150 150 150"   // grey for zero line
...
keep if horizon >= 0   // main horizons only for Figure 1

twoway ///
    (rarea lo95 hi95 horizon, color("`c_all'%15") lwidth(none))          ///
    (rarea lo90 hi90 horizon, color("`c_all'%25") lwidth(none))          ///
    (connected b horizon,                                                  ///
    ...
    subtitle("All episodes (N = 61), 52 EM economies, 1994–2025",         ///
             size(small))                                                  ///
    note("Local projections (Jordà 2005). Driscoll-Kraay SE."             ///
         "Country and year FE. Controls: lagged GDP growth, CA/GDP,"      ///
         "debt/GDP, inflation, spread, IMF program, VIX, US 10yr yield.", ///
         size(vsmall))                                                     ///
...
twoway ///
    ...
    xtitle("Years after crisis onset", size(medsmall))                     ///
    ytitle("Cumulative change in log real GDP p.c. (pp)", size(medsmall))  ///
    title("Output Cost: Non-Default vs. Default-Linked Crises",            ///
          size(medium))                                                     ///
...
*   Shows full event window: h = -2, -1, 0, 1, 2, 3, 4
*   Pre-trend coefficients near zero validates identification
* ════════════════════════════════════════════════════════════════════════════
...
twoway ///
    ...
    subtitle("All spread crises (N = 61). Placebo: h = −2, −1.",          ///
             size(small))                                                   ///
