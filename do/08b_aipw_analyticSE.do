/*===========================================================================
  08B_AIPW_ANALYTICSE.DO — DUPLICATE OF 08b_aipw.do, PAPER-ALIGNED SE VARIANT

  This is a standalone duplicate of 08b_aipw.do, NOT wired into 00_master.do
  and NOT a replacement for it. The ONE change: the LEVEL (nd/def) confidence
  intervals below use the paper's own ANALYTIC (unclustered influence-
  function) SE formula -- i.e. the `A1'/`A2' values _aipwpair already
  computes and returns -- instead of 08b_aipw.do's ADOPTED row-bootstrap SE
  (`BSE1'/`BSE2'). Built to see directly what changes if this project aligns
  to the paper's own construction here, after a diagnostic found the
  analytic formula understated the def arm's true uncertainty by 3.75-5.5x
  on this project's thin default sample (see 08b_aipw.do's own header for
  the full argument -- unchanged, still true; this file exists to look at
  the alternative side by side, not to dispute that finding).
...
/*===========================================================================
  ...
  INFERENCE:
    ...
    - Act 2 (by resolution type) and its difference: each level's CI is
      ...
      far better for them). See git history for the diagnostic run that
      established this (analytic-vs-bootstrap and analytic-vs-clustered SE
      comparisons); those comparison lines are no longer printed each run,
      the decision they informed is now the adopted design. The difference
      (def - nd) is bootstrapped with ROW-LEVEL
...
program define _aipwpair, rclass
    ...
    * & and | equal precedence, left to right.
    ...
    tempname pf
    tempfile bf
    * t1/t2 (each cell's own bootstrap draw) kept, not just their difference --
    * DIAGNOSTIC-ONLY: lets the caller compute each level's own BOOTSTRAP SE,
    * to compare against the adopted analytic SE (a1/a2). See _aipw's header
    * for why the analytic formula can understate uncertainty on a thin
    * treated group; this is the direct test of that concern.
    ...
    preserve
        ...
        if `nd' >= 50 {
            ...
            local bse1 = r(sd)   // DIAGNOSTIC-ONLY: bootstrap SE, cell 1 (default-linked)
            quietly summarize t2
            local bse2 = r(sd)   // DIAGNOSTIC-ONLY: bootstrap SE, cell 2 (non-default)
        }
    restore
    return scalar ok = 1
    return scalar dh = `dh'
...
* ══════════════════════════════════════════════════════════════════════════
di as result _n "=== ACT 2 (PAPER-ALIGNED SE DUPLICATE) — AIPW output cost by resolution, levels + difference ==="
di as result "h   ND (se_analytic)  DEF (se_analytic)  def-nd   [95% boot CI]      Clogg z    p     draws"
di as result "    se_analytic = the PAPER'S OWN analytic (unclustered influence-function) SE formula --"
di as result "    08b_aipw.do's own headline instead ADOPTS a row-bootstrap SE here, after direct"
di as result "    diagnostic evidence that this analytic formula understates the def arm's true"
di as result "    uncertainty by 3.75-5.5x on this project's ~20-episode default arm. This duplicate"
...
forvalues h = 0/4 {
    ...
    * PAPER-ALIGNED (this duplicate only): level CIs = theta +/- 1.96*ANALYTIC
    * SE, matching Asonuma et al.'s own Table 2/Fig. 4 construction literally.
    * 08b_aipw.do's own header documents why this project departed from it
    * (the analytic formula understated the def arm's true uncertainty by
    * 3.75-5.5x on this project's ~20-episode default arm) -- that finding is
    * unchanged; this file exists only to show what the paper's own formula
    * would report here, side by side.
    ...
    * PAPER-ALIGNED (this duplicate only): conventional t-test for each level
    * vs zero using the ANALYTIC SE (b / own analytic SE), matching the
    * paper's own construction rather than this project's adopted bootstrap SE.
...
preserve
    ...
    export delimited "$tabs/aipw_act2_diff_analyticSE.csv", replace
    di as result "AIPW extra-cost-of-default CSV saved: $tabs/aipw_act2_diff_analyticSE.csv"
restore
...
preserve
    ...
    * "all A1" to this foreach and un-silencing Figure A below.)
    ...
    export delimited "$tabs/aipw_results_analyticSE.csv", replace
    di as result "AIPW results CSV saved: $tabs/aipw_results_analyticSE.csv"
    ...
    * "Two AIPW level IRFs; shaded = 1.96*ANALYTIC SE band (the paper's own formula -- 08b_aipw.do's headline instead adopts a row-bootstrap band, see that file's header). Gap = extra cost of default, still bootstrapped directly (row-level), unchanged from the headline. <gapnote>"
    graph export "$figs/fig_aipw_act2_analyticSE.pdf", replace
    di as result "Figure saved: fig_aipw_act2_analyticSE.pdf"
restore

di as result _n "08b_aipw_analyticSE.do complete (paper-aligned SE duplicate)."
di as result "fig_aipw_act2_analyticSE.pdf = the same two-line resolution split as"
di as result "08b_aipw.do's own fig_aipw_act2.pdf, but with 1.96*ANALYTIC SE bands"
di as result "instead of the adopted row-bootstrap bands -- compare the two figures"
di as result "directly to see what the paper's own formula would report here."
di as result "aipw_results_analyticSE.csv / aipw_act2_diff_analyticSE.csv mirror"
di as result "08b_aipw.do's own aipw_results.csv / aipw_act2_diff.csv, se column"
di as result "swapped to analytic; the difference (def-nd) bootstrap itself is UNCHANGED,"
di as result "since that already matched the paper's own bootstrap device."
