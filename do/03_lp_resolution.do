...
program define _nepcount, rclass
    ...
    return scalar n = r(N)
    * Distinct countries contributing this arm's treated onsets -- the flow
    * tier's own per-arm diagnostics (26_lp_debtcrisis_flow.do's _nflowcount)
    * report this alongside episodes; ported here for the same reason: N and
    * episode counts both understate how few independent countries a
    * coefficient actually rests on.
    tempvar tagcty
...
forvalues h = 0/4 {
    ...
    * missing control is dropped by listwise deletion and identifies
    * nothing, which is why the old counts (35/16) disagreed with the
    * treated counts the probit reports.
    _nepcount onset_nd,  outcome(dy_`h') controls(`controls')
    local nepnd = r(n)
    local nctynd = r(ncty)
    _nepcount onset_def, outcome(dy_`h') controls(`controls')
...
forvalues h = 0/4 {
    local hd = `h' + 1
    local row = `h' + 3
    local pnd_`hd'  = 2*(1 - normal(abs(b_nd[`row',1]/se_nd[`row',1])))
    local pdef_`hd' = 2*(1 - normal(abs(b_def[`row',1]/se_def[`row',1])))
}

local lbl_nd  "Non-default"
local lbl_def "Default-linked"
foreach key in nd def {
    file write t2tab "{\b `lbl_`key''}\par" _n
    ...
    forvalues h = 1/5 {
        ...
        local st = r(stars)
        local rr = `h' + 2
        local bstr : display %5.2f b_`key'[`rr',1]
