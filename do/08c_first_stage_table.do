/*===========================================================================
  ...
  Probit with country FE (i.cid, = the paper's c1-c74 in $convar); never-treated
  countries are perfectly predicted and dropped (mechanical ever-treated restriction).
  No year FE.
...
program define _fscol
    ...
    * Country FE (i.cid, = the paper's c1-c74 in $convar). Never-treated countries
    * are perfectly predicted and dropped by probit — the mechanical ever-treated
    * restriction; the surviving-country count shows in the table's Observations row.
...
capture esttab fs_all fs_nd fs_def using "$tabs/table_first_stage.rtf", replace ///
    ...
    addnotes("Dependent variable: dummy = 1 in the onset year of the indicated crisis type; each type predicted vs tranquil years (the rival onset type is dropped)." ///
             "Probit with country fixed effects (i.cid, = the paper's c1-c74). Never-treated countries are perfectly predicted and dropped (mechanical ever-treated restriction). Robust standard errors clustered by country in parentheses." ///
