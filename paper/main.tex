...
($h=1$) following default-linked crises, by around 7 pp -- a level
estimate that is itself significant -- while remaining close to zero and
not significant following non-default crises. The difference between the
two crisis types is significant under the bootstrap test only in the
crisis year ($h=1$). By $h=5$, the pattern has reversed: the level
estimate for default-linked crises is significantly negative, around
$-3$ pp, compared with a small, not significant decline for non-default
crises, so claims on government are then estimated to be lower for
default-linked crises than for non-default crises. This later-horizon
gap is not significant under the bootstrap test, however, so the paper
does not establish whether default-linked crises end with more or less
bank exposure to the sovereign than non-default crises.
