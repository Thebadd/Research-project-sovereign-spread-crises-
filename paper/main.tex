...
open. Finally, our design does not control for the intensity of the
initial spread spike, so part of the contrast between crisis types
could reflect that more severe spread episodes are more likely to end
in default, rather than default itself driving the difference in
outcomes; disentangling the severity of the initiating shock from the
resolution strategy chosen is a natural extension of this work.
...
Third, this paper relates to the literature on the output cost of
sovereign default. \citet{borensztein2009costs} estimate the output
cost of default using a cross-country panel of over 70 countries from
1972 to 2000, and find that default reduces average annual per capita
GDP growth by 1.2 percentage points, with the effect concentrated in
the default's onset year and not statistically significant thereafter.
\citet{furceri2012costly} estimate a considerably larger and more
persistent cost using a broader country panel, finding that output
falls by roughly 8 percent on impact and remains around 10 percent
below its pre-crisis trend eight years after the crisis. \citet{trebesch2017output} compare ``hard'' defaults, involving unilateral
or coercive action by the sovereign, against ``soft'' defaults
negotiated cooperatively, and find that hard defaults carry
significantly higher output costs. \citet{kuvshinov2019sovereigns}
estimate the cost of default net of selection into default and show
that the resulting output loss, roughly 2.7 percent on impact and 3.7
percent at its five-year peak, rises to about 9.5 percent when the
default coincides with a systemic banking crisis. \citet{asonuma2024costs} compare preemptive restructurings, negotiated
before any missed payment, against post-default restructurings, and
find that post-default restructurings are markedly more costly,
particularly in countries with large domestic banking sectors. Our
analysis takes \citet{asonuma2024costs} as a reference point but asks a
different question: whether crises that culminate in sovereign default
carry different macroeconomic costs than crises that do not end in
default at all. \citet{levyyeyati2011elusive} show that a substantial
share of this output contraction occurs before the default itself,
during the period of rising distress that precedes it, a finding that
motivates comparing default-linked and non-default crises within the
same identification framework rather than comparing default to
tranquil years alone.
