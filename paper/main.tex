...
default-linked crises (about 7 pp, significant), against a small but
also significant decline for non-default crises (less than 1 pp) --
the only gap in the paper significant under both tests, and under the
bootstrap test only at this horizon. The increase then fades and turns
negative by $h=5$
(about $-$3 pp, against near zero for non-default crises), but this
reversal is not significant under the bootstrap test at any horizon
beyond $h=1$, so the paper cannot establish whether default-linked
crises end with more or less bank exposure to the sovereign than
non-default crises.
