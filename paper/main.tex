...
The first-stage predictors above capture pre-crisis risk factors, but not
the intensity of the spread shock itself. Table~\ref{tab:spreadintensity}
and Figure~\ref{fig:spreadintensity} report a robustness check that
addresses this gap directly, comparing default-linked onsets against
non-default onsets rather than against tranquil years. Figure~\ref{fig:spreadintensity}
shows that the distribution of peak onset-year spreads for default-linked
crises sits visibly above that for non-default crises, with substantially
more dispersion. Evaluated at the
sample means of the two groups, a univariate probit of default-linked
resolution on peak spread alone (not tabulated) implies a
default-linked resolution probability of approximately 28 percent for a
non-default-sized shock and 52 percent for a default-linked-sized shock.
Table~\ref{tab:spreadintensity} points to a coherent set of economic mechanisms behind this
gap, rather than an isolated statistical association. Higher trade
openness is associated with a greater likelihood of default-linked
resolution. This suggests that more open economies are more exposed to
external terms-of-trade and demand shocks that can precipitate
...
resolution, suggesting greater capacity to absorb the shock through
domestic financial intermediation rather than external restructuring --
the same channel underlying the sovereign-bank nexus results in
Section~\ref{sec:nexus}. A sharper nominal depreciation is associated
with a higher likelihood of default-linked resolution, pointing to a
balance-sheet channel through which currency depreciation raises the
local-currency burden of foreign-currency debt service. Lower
pre-crisis inflation is likewise associated with a higher likelihood of
default-linked resolution. This may reflect inflation acting as a
competing adjustment margin that, where available, erodes the real
value of domestic-currency debt and reduces the need for outright
default. GDP growth, the public debt-to-GDP ratio, the banking
crisis dummy, and government expenditure are not individually
significant, echoing Table~\ref{tab:probit}'s own finding that the
public debt-to-GDP ratio does not distinguish default-linked from
non-default onsets. Taken together, these results indicate that
default-linked and non-default crises differ systematically in the
severity of the initiating shock and in several structural
characteristics -- external exposure, financial depth, and currency
mismatch -- rather than in conventional fiscal fragility indicators,
and motivate the caveat discussed in the Introduction.
