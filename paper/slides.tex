...
\begin{frame}{Main specification: two stages}
  ...
  \begin{itemize}
    ...
    rate)
    \item $Z_{i,t}$: 3 excluded predictors -- shift crisis probability
    but are not shaped by the country's own crisis (US federal funds
    rate, a contagion measure, years since the country's last
    default-linked onset)
  \end{itemize}
  \vspace{0.4em}
  ...
  \begin{itemize}
    ...
    ($X_{i,t}$: same baseline controls as Stage 1)
  \end{itemize}
  \vspace{0.4em}
  \begin{center}
    \small
...
\begin{frame}{Why a first-stage probit?}
  ...
  \]
  \begin{itemize}
    \item $S \in \{$non-default, default-linked$\}$; $X_{i,t}$ and
    $Z_{i,t}$ as defined on the previous slide
  \end{itemize}
\end{frame}
