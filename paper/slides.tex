...
\begin{frame}{Main Specification: outcome model}
  ...
  \begin{itemize}
    ...
    as if $D=1$, once as if $D=0$ -- to get two numbers:
    $\widehat{m}_1^{S,h}$ and $\widehat{m}_0^{S,h}$
...
\begin{frame}{Main Specification: the AIPW estimator}
  \small
  Two ingredients so far: $\widehat{m}_1^{S,h}, \widehat{m}_0^{S,h}$
  (outcome model) and $\widehat{p}_{i,t+1}^S$ (probit). We combine them,
  following \citet{jorda2016time}:
  \vspace{0.2em}
  \footnotesize
  \[
  \Lambda_h^S = \frac{1}{N}\sum_{i}\sum_{t}\Biggl\{
  \underbrace{\left[\frac{D_{i,t+1}^S(y_{i,t+h}-y_{i,t})}{\widehat{p}_{i,t+1}^S}
    - \frac{(1-D_{i,t+1}^S)(y_{i,t+h}-y_{i,t})}{1-\widehat{p}_{i,t+1}^S}\right]}_{\text{(a) reweighted gap}}
  - \underbrace{\frac{D_{i,t+1}^S - \widehat{p}_{i,t+1}^S}{\widehat{p}_{i,t+1}^S(1-\widehat{p}_{i,t+1}^S)}
    \left[(1-\widehat{p}_{i,t+1}^S)\widehat{m}_1^{S,h} + \widehat{p}_{i,t+1}^S\widehat{m}_0^{S,h}\right]}_{\text{(b) correction}}
  \Biggr\}
  \]
  \small
  \vspace{0.2em}
  \begin{itemize}
    \item $\Lambda_h^S$: our result -- the effect of crisis type $S$ on
    the outcome at horizon $h$
    \item (a): the raw outcome gap between crisis and non-crisis
    observations, reweighted by how predictable each one was
    \item (b): a correction term, built from the outcome model, for
    whatever the probit gets wrong
    \item Why bother with (b): the result stays reliable even if the
    probit is imperfect, as long as the outcome model is right (or
    vice versa) -- ``doubly robust''
