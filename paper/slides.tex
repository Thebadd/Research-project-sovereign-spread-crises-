...
\begin{frame}{Main specification: two stages}
  \footnotesize
  \textbf{Stage 1 -- probit:} estimate each country-year's ex-ante
  probability of entering a non-default or default-linked spread crisis
  \[
  \Pr\!\left(D_{i,t+1}^S = 1 \mid X_{i,t}, Z_{i,t}\right) = \Phi\!\left(X_{i,t}\gamma^{S,X}, Z_{i,t}\gamma^{S,Z}\right)
  \]
  \vspace{-0.2em}
  \begin{itemize}
    \item $\to$ predicted probability $\widehat{p}_{i,t+1}^S$
  \end{itemize}
  \vspace{0.4em}
  \textbf{Stage 2 -- AIPW local projection:} use $\widehat{p}_{i,t+1}^S$
  to reweight observations and estimate the cumulative effect of each
  crisis type at every horizon $h$
  \[
  y_{i,t+h} - y_{i,t} = \alpha_i^{S,h} + \beta^{S,h} D_{i,t+1}^{S} + X_{i,t}\beta^{S,h} + \mu_{i,t+h}^{S}
  \]
  \vspace{-0.2em}
  \begin{itemize}
    \item $\to$ post-crisis trajectories, non-default vs.\ default-linked
  \end{itemize}
  \vspace{0.4em}
  \begin{center}
    \small
    Stage 1 (probit) $\;\to\;$ IPW weights $\;\to\;$ Stage 2 (local
    projection) $\;\to\;$ effect by horizon $h$
  \end{center}
\end{frame}
...
\begin{frame}{Controls $X_{i,t}$ and predictors $Z_{i,t}$}
  \small
  \textbf{Baseline controls $X_{i,t}$} (same set in every specification,
  probit and local projection):
  \begin{itemize}
    \item GDP growth, public debt-to-GDP ratio, banking crisis dummy,
    government expenditure-to-GDP ratio, trade openness, bank
    credit-to-GDP ratio, inflation, nominal exchange rate
  \end{itemize}
  \vspace{0.4em}
  \textbf{Predictors $Z_{i,t}$} (probit only -- satisfy an exclusion
  restriction: they shift a country's probability of entering a crisis
  but are not themselves shaped by that country's own crisis):
  \begin{itemize}
    \item US federal funds rate (lagged)
    \item Contagion: share of a country's neighborhood in crisis,
    weighted by inverse geographic distance (lagged)
    \item Years since the country's own most recent default-linked
    spread crisis onset
  \end{itemize}
\end{frame}
...
% ---------------- AIPW LP: specification ----------------
\begin{frame}{Second stage: AIPW local projection}
  \[
  y_{i,t+h} - y_{i,t} = \alpha_i^{S,h} + \beta^{S,h} D_{i,t+1}^{S} + X_{i,t}\beta^{S,h} + \mu_{i,t+h}^{S}
  \]
  \vspace{0.3em}
  \begin{itemize}
    \item $y_{i,t+h} - y_{i,t}$: cumulative change in the outcome from
    crisis onset to horizon $h$
    \item $D_{i,t+1}^{S}$: onset dummy for crisis type $S \in$
    \{non-default, default-linked\}
    \item $X_{i,t}$: pre-crisis controls; $\alpha_i^{S,h}$: country
    fixed effects
    \item Estimated using the first-stage probabilities to reweight
    observations, following \citet{jorda2016time}, separately for each
    crisis type and horizon $h$
