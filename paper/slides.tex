...
\begin{frame}{Preview of results}
  \small
  \begin{itemize}
    \item \textbf{Crisis type is predictable}: larger spread shocks,
    more trade openness, sharper depreciation $\to$ default; deeper
    bank credit, higher inflation $\to$ non-default
    \item \textbf{GDP}: $-$1--2 pp (non-default) vs.\ $-$4--9 pp (default)
    \item \textbf{Bank credit}: +7 pp (non-default) vs.\ $-$40+ pp
    (default)
    \item \textbf{Investment, FDI, lending rates}: no significant
    difference by crisis type
    \item \textbf{Claims on government}: +7 pp vs.\ $<$1 pp --
    only outcome robust under both tests
    \item \textbf{Nexus}: matters for non-default (0.75 vs.\ $-$2.87 pp,
    sig.), not for default (18 vs.\ 8 pp, not sig.)
  \end{itemize}
\end{frame}
