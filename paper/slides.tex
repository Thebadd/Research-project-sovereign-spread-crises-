...
\begin{frame}{Main Specification: Probit model}
  ...
  non-default or default-linked spread crisis
  \[
  ...
  \begin{itemize}
    ...
    \item $\to$ we derive the predicted probability
    $\widehat{p}_{i,t+1}^S$
  \end{itemize}
\end{frame}

% ---------------- Main Specification: AIPW local projection ----------------
% ---------------- Probit: results ----------------
...
% ---------------- Main Specification: AIPW local projection ----------------
\begin{frame}{Main Specification: AIPW local projection}
  \footnotesize

  We use $\widehat{p}_{i,t+1}^S$ to reweight observations and estimate
  the cumulative effect of each crisis type on the outcome at every
  horizon $h$
  \[
  ...
  \begin{itemize}
    ...
    \item $\beta^{S,h}$: coefficient of interest -- the effect of a
    crisis of type $S$ on the outcome at horizon $h$
    \item $\mu_{i,t+h}^{S}$: error term
    \item Estimated using $\widehat{p}_{i,t+1}^S$ (Stage 1) to reweight
