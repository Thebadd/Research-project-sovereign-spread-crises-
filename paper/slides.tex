...
\begin{frame}{Main Specification: outcome model}
  \small
  One regression per crisis type and horizon: the same equation as
  before, but now weighted so that predictable observations count less
  \[
  y_{i,t+h} - y_{i,t} = \alpha_i^{S,h} + \beta^{S,h} D_{i,t+1}^{S} + X_{i,t}\beta^{S,h} + \mu_{i,t+h}^{S}
  \]
  \vspace{-0.2em}
  \begin{itemize}
    \item Same variables as the probit stage: $y$ = outcome change,
    $D$ = onset dummy, $X$ = controls, $\alpha_i$ = country fixed
    effects
    \item Weight each observation by
    $w_{i,t} = D_{i,t+1}^S/\widehat{p}_{i,t+1}^S + (1-D_{i,t+1}^S)/(1-\widehat{p}_{i,t+1}^S)$,
    using $\widehat{p}_{i,t+1}^S$ from Stage 1
    \item Then predict twice from that one fitted regression -- once
    as if $D=1$, once as if $D=0$ -- to get two numbers per
    country-year: $\widehat{m}_1^{S,h}$ = ``predicted outcome if this
    country-year had this crisis type'', $\widehat{m}_0^{S,h}$ =
    ``predicted outcome if it did not''
    \item These two numbers feed into the next slide; they are not
...
\begin{frame}{Main Specification: the AIPW estimator}
  \footnotesize
  For each country-year, take (a) $-$ (b) below; the result
  $\Lambda_h^S$ is the average of that quantity over every
  country-year in the sample -- our estimated effect of crisis type
  $S$ on the outcome at horizon $h$:
  \vspace{0.2em}
  \[
  \Lambda_h^S = \frac{1}{N}\sum_{i}\sum_{t}\Biggl\{
  \underbrace{\left[\frac{D_{i,t+1}^S(y_{i,t+h}-y_{i,t})}{\widehat{p}_{i,t+1}^S}
    - \frac{(1-D_{i,t+1}^S)(y_{i,t+h}-y_{i,t})}{1-\widehat{p}_{i,t+1}^S}\right]}_{\text{(a)}}
  - \underbrace{\frac{D_{i,t+1}^S - \widehat{p}_{i,t+1}^S}{\widehat{p}_{i,t+1}^S(1-\widehat{p}_{i,t+1}^S)}
    \left[(1-\widehat{p}_{i,t+1}^S)\widehat{m}_1^{S,h} + \widehat{p}_{i,t+1}^S\widehat{m}_0^{S,h}\right]}_{\text{(b)}}
  \Biggr\}
  \]
  \vspace{0.2em}
  \begin{itemize}
    \item \textbf{(a) for a country-year that actually had this
    crisis} ($D=1$): its own observed outcome change, divided by
    $\widehat{p}_{i,t+1}^S$ -- the rarer/less predictable this crisis
    was for it, the more it counts
    \item \textbf{(a) for a country-year that did not} ($D=0$): minus
    its own observed change, divided by $1-\widehat{p}_{i,t+1}^S$
    \item \textbf{(b), the correction:} $D_{i,t+1}^S - \widehat{p}_{i,t+1}^S$
    is the probit's own prediction error for that country-year (0 or 1
    minus the predicted probability); (b) uses the outcome model's
    $\widehat{m}_1,\widehat{m}_0$ to correct for that error, so a
    probit mistake does not translate one-for-one into a mistake in
    $\Lambda_h^S$
    \item \textbf{Why bother with (b):} the average still comes out
    right even if the probit is somewhat wrong, as long as the outcome
    model is right (or vice versa) -- ``doubly robust''
    \item Differences between non-default and default-linked estimates
    are tested with a paired bootstrap (governing) and the
    \citet{clogg1995statistical} test (companion)
  \end{itemize}
\end{frame}
