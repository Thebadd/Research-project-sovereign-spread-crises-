...
\begin{frame}{Main Specification: outcome model}
  ...
  For each crisis type $S$ and horizon $h$, we estimate the outcome
  regression by OLS, separately for the treated ($D=1$) and untreated
  ($D=0$) states
  \[
  ...
  \begin{itemize}
    ...
    \item $\to$ this regression yields the predicted conditional means
    $\widehat{m}_1^{S,h}$ (treated) and $\widehat{m}_0^{S,h}$
    (untreated) -- these are inputs to the AIPW estimator, not the
    final effect
  \end{itemize}
\end{frame}
