...
\begin{frame}{Identification: two-step AIPW}
  \small
  \begin{itemize}
    \item \textbf{Step 1 -- probit (propensity model):} estimate the
    probability that a country-year is a crisis onset, separately for
    non-default and default-linked crises, from pre-crisis controls
    plus three excluded predictors (US federal funds rate, a
    contagion measure, years since the country's last default-linked
    onset)
    \item \textbf{Step 2 -- AIPW local projection:} use the predicted
    probabilities to reweight observations by how predictable their
    crisis type was ex ante, combined with an outcome regression,
    following \citet{jorda2016time}
    \item This corrects for the fact that crises are not randomly
    assigned: countries entering a given crisis type are systematically
    different on observables, so a simple mean comparison or OLS would
    conflate selection with the effect of the crisis itself
  \end{itemize}
\end{frame}
