...
> **Empirical link — Non-default credit channel.** The local-projection estimates show that **private credit/GDP falls by −4.78 pp at $h=4$** in non-default episodes. The empirical variable is the ratio $\ell_t / Y_t$, measured in the data as private credit as a share of GDP.
>
> In the non-default path, the **credit channel is the dominant transmission mechanism**: the sovereign spread rise erodes bank net worth $N_t$ through (LL.5a), and the leverage constraint (16) forces banks to cut private lending $\ell_t$ in proportion — equation (IR.4). The key fact is that **$\ell_t$ falls faster than $Y_t$**: the output loss is moderate (−3.76 pp at $h=1$), but the credit collapse is much larger in proportional terms. As a result, the ratio $\ell_t / Y_t$ falls substantially — the numerator drives the result.
>
> Numerically: if GDP falls by −3.8 pp and credit (in levels) falls by −8 pp, the ratio credit/GDP falls by approximately −4.4 pp. The credit contraction is progressive (negligible at $h=0$, deepening to $h=4$) because net-worth erosion accumulates over time as the spread remains elevated. **In non-default episodes, $\ell_t$ falls faster than $Y_t$ → the ratio falls.**
...
> **Empirical link — Default credit channel (the GDP-denominator effect).** The local-projection estimates show that **private credit/GDP contracts significantly *less* in default-linked episodes than in non-default episodes**, and is often not statistically distinguishable from zero. This result requires careful interpretation because the empirical variable is the **ratio** $\ell_t / Y_t$, not the level of credit.
>
> The explanation is a **GDP-denominator effect**: under default, the output collapse is far more severe than under non-default (−5.50 pp vs. −3.76 pp at $h=1$, and deepening via capital depletion at $h=2,3,4$). Even if private credit in levels falls significantly, GDP falls *even faster* because the dominant transmission mechanism in default is **capital depletion (IR.5)** — $I_t = 0$ during exclusion, so $K_t$ shrinks at −10%/year, pulling output down independently of the credit channel.
>
> **Numerical illustration:** suppose at $h=3$, private credit falls by −6 pp (in levels) but GDP falls by −8 pp (from capital depletion). Then the ratio $\ell/Y$ actually *rises* (+2 pp) — the denominator effect dominates. This is precisely why the LP coefficient on private credit/GDP is smaller (or close to zero) in default episodes, even though the underlying private credit collapse is more severe in levels.
>
> **The lesson:** the credit/GDP ratio falls more in non-default episodes because in that path **credit is the channel** — $\ell_t$ falls faster than $Y_t$. In default episodes, **output is the channel** — $Y_t$ falls faster than $\ell_t$, so the ratio does not worsen as much. The two paths exhibit fundamentally **different transmission mechanisms**: credit-driven recession under non-default; capital-depletion-driven recession under default.

> **Proposition 3 (Credit/GDP Paradox).** Private lending in *levels* collapses more severely under default ($\ell^{\text{def}}_t = 0$) than under non-default ($\ell^{nd}_t > 0$). Yet the empirically measured ratio **private credit/GDP falls *less* under default**. The resolution is the **GDP-denominator effect**: the output loss under default is driven by capital depletion (IR.5) — $Y_t$ falls much faster than under non-default — so the denominator shrinks at least as fast as the numerator, leaving the ratio approximately unchanged. Under non-default, by contrast, $\ell_t$ falls faster than $Y_t$ (the credit channel dominates output), so the ratio falls sharply. **Conclusion:** a smaller measured credit/GDP response in default episodes is not evidence that banks are lending more — it is evidence that GDP is collapsing faster than credit, driven by a completely different transmission mechanism (capital depletion, not credit crunch).
