...
> **Empirical link — Non-default credit channel.** The local-projection estimates show that **private credit/GDP falls by −4.78 pp at $h=4$** in non-default episodes. This is the dominant transmission channel for this path: the sovereign spread rise erodes bank net worth $N_t$ through (LL.5a), and the leverage constraint (16) forces banks to cut private lending $\ell_t$ in proportion — equation (IR.4) quantifies exactly this mapping. The credit contraction is progressive (small at $h=0$, deepening to $h=4$) precisely because net-worth erosion accumulates over time as the spread remains elevated. **The non-default path is the path where the credit channel is empirically active and statistically significant.**
...
> **Empirical link — Default credit channel.** The local-projection estimates show that **private credit/GDP contracts significantly *less* in default-linked episodes than in non-default episodes**, and is not statistically distinguishable from zero in several specifications. This is not a puzzle — it is the **direct empirical signature of equation (LL.6D) combined with the balance-sheet reallocation**. During default, banks' total assets remain at $\lambda N_t$ (the leverage constraint stays binding), but their composition switches entirely from private loans to sovereign bonds: $b^B_t = \lambda N_t$, $\ell_t = 0$. An econometrician measuring *aggregate bank credit to the private sector* observes the collapse in $\ell_t$, but this is partly offset in credit aggregates that include claims on government. More importantly, the **output loss in default episodes does not operate through the domestic credit channel** — it operates through capital depletion (IR.5, Break 3 below) and the autarky lending-rate wedge $\Delta r^{L,\text{aut}}$. The two paths therefore exhibit fundamentally **different transmission mechanisms**, not just different intensities of the same one.

> **Proposition 3 (Credit Paradox).** For all $h \ge 0$, private lending under the default path is strictly lower than under the non-default path ($\hat{\ell}^{\text{def}}_h \ll \hat{\ell}^{nd}_h < 0$). Yet the private-credit contraction in aggregate data may appear smaller under default: banks do not reduce total assets (gross assets remain $\lambda N_t$), they merely redirect them from private loans to sovereign bonds. The econometrician measuring *total bank credit* (including government) observes a reallocation, not a deleveraging, so the *private-credit* squeeze is invisible in credit aggregates. This is why default-linked episodes show no statistically larger measured private-credit contraction despite the doom loop. **The dominant transmission channel in the default path is capital depletion (IR.5), not credit (IR.4).**
